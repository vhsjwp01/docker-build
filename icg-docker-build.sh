        ...
        case "${this_git_action}" in 
            ...
            checkout)
                ...
                if [ "${GIT_CHECKOUT_BASE}" != "" -a "${stash_project}" != "" -a "${2}" != "" ]; then
                    ...
                    cd "${GIT_CHECKOUT_BASE}/${stash_project}"
                    let branch_check=`${my_git} branch | ${my_egrep} -c "\ ${this_branch}$"`
...
if [ ${exit_code} -eq ${SUCCESS} ]; then
    ...
    for command in awk curl docker egrep git host jq mkdir rm sed sort tail wc ; do
...
if [ ${exit_code} -eq ${SUCCESS} ]; then
    ...
    while (( "${#}" )); do
        ...
        case "${1}" in
            ...
            --docker_registry|--docker_namespace|--docker_build_args|--git_branch|--stash_project|--personal_stash_project)
    ...
    if [ "${docker_namespace}" != "" ]; then
        ...
        let ns_length=`echo -ne "${docker_namespace}" | ${my_wc} -c`
...
if [ ${exit_code} -eq ${SUCCESS} ]; then
    ...
    if [ "${GIT_CHECKOUT_BASE}/${stash_project}/Dockerfile" ]; then
        ...
        echo "Performing docker build against ${GIT_CHECKOUT_BASE}/${stash_project}/Dockerfile ... see ${artifact_file} for status logs"
...
if [ ${exit_code} -eq ${SUCCESS} ]; then
    ...
    if [ ${local_image_check} -eq 0 ]; then
        echo "Tagging newly created container id ${container_id} as: ${docker_namespace}/${stash_project}:${docker_image_tag}"
        ...
        echo "Docker image tag ${docker_namespace}/${stash_project}:${docker_image_tag} already exists ... no action taken"
    fi
...
if [ ${exit_code} -eq ${SUCCESS} ]; then
    ...
    if [ "${docker_registry}" != "" ]; then
        ...
        if [ ${remote_image_check} -eq 0 ]; then
            echo "Adding new image ${docker_namespace}/${stash_project}:${docker_image_tag} to ${docker_registry_uri}"
            ${my_docker} tag ${container_id} ${docker_registry_uri}/${docker_namespace}/${stash_project}:${docker_image_tag} &&
            ${my_docker} push ${docker_registry_uri}/${docker_namespace}/${stash_project}:${docker_image_tag}
        else
            ...
            while [ ${remote_image_counter} -ge 0 ]; do
                ...
                if [ "${this_image_name}" = "${docker_namespace}/${stash_project}" ]; then
                    ...
                    for remote_image_tag in ${remote_image_tags} ; do
                        ...
                        if [ "${this_image_name}:${remote_image_tag}" = "${docker_namespace}/${stash_project}:${docker_image_tag}" ]; then
            ...
            if [ ${remote_tag_check} -eq 0 ]; then
                echo "Pushing updated image ${docker_namespace}/${stash_project}:${docker_image_tag} to ${docker_registry_uri}"
                ${my_docker} tag ${container_id} ${docker_registry_uri}/${docker_namespace}/${stash_project}:${docker_image_tag} &&
                ${my_docker} push ${docker_registry_uri}/${docker_namespace}/${stash_project}:${docker_image_tag}
            else
                echo "Docker image tag ${docker_registry_uri}/${docker_namespace}/${stash_project}:${docker_image_tag} already exists ... no action taken"
            fi
